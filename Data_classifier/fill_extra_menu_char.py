...
def get_keyword_charac():
    ...
    load_wb = openpyxl.load_workbook('../DB_cell/extra_menu_key.xlsx', data_only=True)
    load_ws = load_wb['menu_key']
    for row in load_ws.iter_rows(min_row=2):
        key_charac = []
        key_charac.append(['isSoup',row[1].value])
...
def get_extra_menu():
    load_wb = openpyxl.load_workbook('../DB_cell/extra_menu_data.xlsx', data_only=True)
...
def extract_menu(menu_charac):
    write_wb = openpyxl.load_workbook('../DB_cell/extra_menu_data.xlsx', data_only=True)
